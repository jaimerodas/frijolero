...
#
# Start: if there are uncommitted edits, commit, rebase, push. Pull either
# way, so fava shows the statements that the droplet added. Then run fava.
# Stop (Ctrl-C): commit and push. The sh wrapper catches the interrupt and
# exits with 0, so fish does not cancel the function and the sync runs.

function moneys --description 'Pull the ledger, run fava, push your edits'
    set -l repo $MONEYS_REPO
    if _moneys_commit $repo
        echo "moneys: committed edits left over from the last session"
        _moneys_pull $repo; and git -C $repo push -q
    else
        _moneys_pull $repo
    end
...
function _moneys_pull --argument-names repo
