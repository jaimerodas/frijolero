...
### Reportes (`app/reports.rb`, `lib/reports.rb`)

`/reports/income` and `/reports/balance` are whole-ledger reports. `Reports` runs `rledger query --no-cache -q -f json` (rustledger, a static binary; `Config.rledger`) on `moneys.beancount` (`Config.report_file`, the fava entry point, so options, opens, balances and prices are in), one BQL query per page, and parses the JSON rows into `{account => {currency => BigDecimal}}`. Both row shapes are accepted: 0.22 prints objects, 0.24 arrays. `income(from, to)` sums Income and Expenses postings in the interval. `balance(at)` values every account with `VALUE(SUM(position), at)` and folds Income plus Expenses up to that day, negated, into one `Equity:Utilidades-acumuladas` row. `tree` adds the parent accounts with subtotals, in tree order, and the views show one column per currency seen, income and liabilities with the sign flipped. Only the first level under each root is visible at load; a parent's name is a button and `public/reports.js` (the one script in the app) shows its direct children or hides and resets its subtree. Dates come from `Date.iso8601` before they touch the query; a bad date falls back to the default (year to date, or today). A `Reports::Error` renders the page with the message and status 502. Measured on the real ledger: 0.07 s and 21 MB per query, so there is no cache. Both pages show the clone's last commit (`LedgerRepo#head`, date and subject) and an "Actualizar ledger" button. It posts to `/ledger/pull`, which runs `repo.pull` and returns to the report with `?pull=ok`, or with the git error in `pull`. That is the third way the droplet's clone catches up with the laptop, besides a job's first pull and the pull inside every commit. rustledger is stricter than Python beancount on one point: it warns when an account closes with a non-zero balance (`Income:Gains:LaTasa`); `-q` keeps that off stderr and the query still runs.
