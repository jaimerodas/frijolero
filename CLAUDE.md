...
**Gráficas** (`app/charts.rb`, `public/charts.js`, `public/d3.min.js`). A report can have a chart block under the toolbar. It exists only when the person asks for it: a "Gráfica" tab at the end of the toolbar sets `?chart=<name>`, `report_query` carries it with the journal filter, and a page without it renders no block, no data and no script. `chart_available?` says which pages offer it; today only the journal of one account, with `history`. Ruby embeds the rows the page already has as JSON in `<script type="application/json" id="chart-data">` (`chart_data`: the period as drawn, a running one cut at today, and the matched postings as date, currency and amount in the report sign, so the bars add up to the total on the page) and `charts.js` aggregates and draws with d3. A new chart on the same rows is JS only; one that needs other data gets its own query in `Reports`, run only when that chart is on. The library is d3 7.9.0, vendored as the UMD `dist/d3.min.js` in `public/` (no node, no bundler, no CDN; the ESM entry on the CDN is a graph of 30 module URLs, so there is no single ESM file) and loaded with classic deferred `<script>` tags before `charts.js`, so `d3` is a global. Upgrading is replacing the file. The histogram offers the buckets finer than the page's resolution (Mes → día; Trimestre → día, mes; Año → día, mes, trimestre; Todo → all four; defaults día, mes, mes, año), as buttons inside the block, client-side and not in the URL; gaps are zero; one figure per currency, with a caption only when there is more than one. Dates are plain days: `d3.utcParse` and the `utc` intervals keep them as written. Charts are SVG with a `viewBox` at the block's width, so they scale on phones, and they take every color and font from `style.css` (`.chart .bar`, `.bar.debit`, `.tick`), never from attributes set in JS, so dark mode and the NASA palette hold. Rack tests cover the toggle and the JSON; the drawing is checked by eye in Safari against the page total. There is no JS test runner by design.
